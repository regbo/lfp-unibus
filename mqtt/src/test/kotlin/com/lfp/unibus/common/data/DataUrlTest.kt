...
class DataUrlTest {
  ...
  fun `parse data URL with base64 as media type`() {
    ...
    assertNotNull(result)
    assertNull(result!!.mediaType) // Media type remains unset when header omits it
    assertEquals(true, result.base64)
    assertArrayEquals("Hello World".toByteArray(), result.data)
  }
  ...
  fun `parse returns null for empty part in header`() {
    ...
    val input1 = "data:text/plain;;base64,SGVsbG8gV29ybGQ="
    // Empty first part (before first semicolon) - this is actually base64 as media type, which is
    // valid
    ...
    val result2 = DataUrl.parse(input2)
    assertNull(result2, "Empty first part should be invalid")
  }
  ...
  fun `parse data URL with special characters in parameters`() {
    val input =
        "data:application/json;param1=value%201;param2=value-2;base64,eyJ0ZXN0IjoidmFsdWUifQ=="
    val result = DataUrl.parse(input)
  ...
  /**
   * Tests toString with non-base64 content. Note: toString() always base64-encodes the data
   * internally, but omits ";base64" if base64=false. This creates a format that may not round-trip
   * correctly since the parser will treat it as URL-encoded.
   */
  @Test
  ...
  /**
   * Tests that malformed base64 returns null gracefully. Note: Base64 decoder may handle some
   * padding issues, so test with clearly invalid base64.
   */
  @Test
